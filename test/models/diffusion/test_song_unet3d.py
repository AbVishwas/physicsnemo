...
from physicsnemo.models.diffusion import SongUNet3D as UNet


@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_song_unet3d_forward(device):
    torch.manual_seed(0)
    # Construct the DDM++ UNet3D model
    model = UNet(img_resolution=8, in_channels=2, out_channels=2).to(device)
    input_image = torch.ones([1, 2, 8, 8, 8]).to(device)
    ...
    assert common.validate_forward_accuracy(
        ...
        file_name="ddmpp_unet3d_output.pth",
        atol=1e-3,
    )

    torch.manual_seed(0)
    # Construct the NCSN++ UNet3D model
    model = UNet(
        img_resolution=8,
        in_channels=2,
        out_channels=2,
    ...
    assert common.validate_forward_accuracy(
        model,
        (input_image, noise_labels, class_labels),
        file_name="ncsnpp_unet3d_output.pth",
        atol=1e-3,
    )


@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_song_unet3d_constructor(device):
    """Test the Song UNet3D constructor options"""
    ...
    # test non-cubic shape (different D, H, W)
    model = UNet(
        img_resolution=[8, 16, 16],
        in_channels=in_channels,
        out_channels=out_channels,
        embedding_type="fourier",
        channel_mult_noise=2,
    ...
    noise_labels = torch.randn([1]).to(device)
    class_labels = torch.randint(0, 1, (1, 1)).to(device)
    input_image = torch.ones([1, out_channels, 8, 16, 16]).to(device)
    output_image = model(input_image, noise_labels, class_labels)
    assert output_image.shape == (1, out_channels, 8, 16, 16)

    # test rectangular shape (D=H, W different)
    model = UNet(
    ...
    input_image = torch.ones(
        [1, out_channels, img_resolution, img_resolution, img_resolution * 2]
    ).to(device)
    output_image = model(input_image, noise_labels, class_labels)
    assert output_image.shape == (
        1,
...
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_song_unet3d_optims(device):
    """Test Song UNet3D optimizations"""
    ...
    # don't activate amp for SongUNet3D on CPU)
    if device == "cuda:0":
...
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_song_unet3d_checkpoint(device):
    """Test Song UNet3D checkpoint save/load"""
    # Construct UNet3D models
    model_1 = UNet(
        img_resolution=8,
        in_channels=2,
        out_channels=2,
...
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_song_unet3d_deploy(device):
    """Test Song UNet3D deployment support"""
    model = UNet(
        img_resolution=8,
        in_channels=2,
        out_channels=2,
...
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
def test_song_unet3d_grad_checkpointing(device):
    channels = 2
    img_resolution = 16
    ...
    input_image = torch.ones(
        [1, channels, img_resolution, img_resolution, img_resolution]
    ).to(device)
    noise_labels = torch.randn([1]).to(device)
    class_labels = torch.randint(0, 1, (1, 1)).to(device)
